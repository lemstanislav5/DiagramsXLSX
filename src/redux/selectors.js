/*
  сначала построим функционал в данном коде
  потом будет посмотреть фсе фризы и скорости выполнения
  убрать лишний рендер
...
export const getFilesData = (state) => {
  return state.files.data.reduce((accumulator, currentValue, index, arrey) => {
...
export const getNodeLines = (state)=> {
  return state.options.contactLines;
}
...
export const sortBySender = createSelector(
  ...
  if(data !== undefined && firstNumber !== undefined && secondNumber !== undefined){
    ...
    /**
   * Получился массив следующих элементов: {sender: '553691******0415', payments: Array(44)}
